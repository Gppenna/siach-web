...
export class AppStateService {
	...
	private state$: BehaviorSubject<any> = new BehaviorSubject<any>(this.initialState);
	...
	private login(credentials: any): Observable<any> {
		return of(
			this.authenticationService.authenticate(credentials).subscribe((response: any) => {
	...
	toggleTheme(): void {
		...
		this.httpRequest({
			...
			path: 'user/tema',
			query: `name=${sessionStorage.getItem(this.USER_EMAIL_SESSION_ATTRIBUTE)}`,
		}).subscribe();
	}
	...
	private loadDependencies(deps: any, refsObject?: any) {
		...
		Object.keys(deps).forEach((key: string) => {
			const path = this.replaceValuesFromSource(deps[key].path, '/', refsObject);
			deps[key].path = path;

			const query = this.replaceValuesFromSource(deps[key].query, '&', refsObject);
	...
	private replaceValuesFromSource(replace: string, split: string = '/', values?: any) {
		if (!replace) {
			return replace;
		}
	...
	private replaceFromSource(replace: string, values?: any) {
		try {
			const ref = replace.indexOf('=') !== -1 ? replace.split('=')[1].replace('#', '') : replace.replace('#', '');
			return replace.indexOf('=') !== -1 ? `${replace.split('=')[0]}=${values[ref]}` : values[ref];
		} catch (error) {
			throw new Error('Configure corretamente os objetos e relações :: [' + replace + '] não existe em ::' + JSON.stringify(values));
		}
	}
}
